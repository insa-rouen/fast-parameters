...
#
# Comments:
#     - 0.0: generate .bts wind file by using TurbSim (retrieve old batch.py)
#     - 0.1: enable multiprocessing
#     - 0.2: create a supervising function for multiprocess
#     - 0.3: [28/12/2018] some small changes
#     - 0.4: [25/04/2019] generate script from a template NTM_*mps.inp
#
# Description:
# 
# 
...
class Turbulence(object):
    ...
    def __init__(self, seed, logpath, echo):
        self.seed = seed
        self.echo = echo
        ...
        # input filename
        self.filename = '{}_*mps.inp'.format(self.seed[0])
        self.logPath = Path(logpath).expanduser()
    ...
    def _make_copy(self):
        ...
        with open(self.filename, 'r') as f:
            ...
            for index, line in enumerate(data):
                if '??' in line:
                    data[index] = re.sub('\?\?', self.seed[1], line)
                if 'RandSeed1' in line:
                    data[index] = self._replace(line, 'RandSeed1', self.seed[2])
                if 'URef' in line:
                    data[index] = self._replace(line, 'URef', self.seed[1])
        # creat new .inp file --------------------------------------------------
        self.filename = '{}_{}mps_{}.inp'.format(self.seed[0], self.seed[1],
                                                 self.seed[2])
    ...
    def _replace(self, string, keyword, value):
        ...
        newtime = re.sub('[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)', str(value),
                         substring)
    ...
    def _turbsim(self, silence):
        ...
        '''
        command = '{0} {1}'.format(self._turbsimName, self.filename)
        try:
            if silence:
        ...
        if wrong:
            ...
            filename = self.logPath.joinpath('{}.TurbSim.bug'.format(
                                             self.seed[2]))
            filename.write_bytes(errorMessage)
            utils.add_failed_seeds('TurbSim', self.seed)
            raise Exception("|- [ERROR] TurbSim has an error for simulation {},"
                            " please see report in {}".format(self.seed,
                            filename))
...
#!                                 FUNCTION DEFINITION
#!------------------------------------------------------------------------------
def get_turbulence(seed, logpath, silence=False, echo=True):
    temp = Turbulence(seed=seed, logpath=logpath, echo=echo)
    temp.run(silence=silence)
    del temp
    return seed

def get_turbulence_multiprocess(list_seed, logpath, silence=False, echo=False):
    ...
    def printer(seed):
        ...
        print("|- [TurbSim][{}/{}] {} wind at {} m/s with seed ID {} is created"
              " at {}:{}. {} tasks waiting to be completed ...".format(pos,
              length, seed[0], seed[1], seed[2], hour, minute, rest))
    # begin multiprocessing
    pool = multiprocessing.Pool()
    [pool.apply_async(get_turbulence, args=(seed, logpath, silence, echo), callback=printer,
