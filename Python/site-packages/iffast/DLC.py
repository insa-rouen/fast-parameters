...
class DLC(object):
    ...
    def _move(self):
        ...
        if self.moveSource:
            source = self.inputPath+'/Wind/{}/{}_{}mps_{}.bts'.format(self.case,
                     self.seed[0], self.seed[1], self.seed[2])
            destination = self.inputPath+'/Wind/{}/finished/'.format(self.case)
            if not os.path.exists(destination): # create new folder if non-exist
...
def get_DLC11_multiprocess(list_seed, outputFolder='/', moveSource=False, silence=False, echo=False):
