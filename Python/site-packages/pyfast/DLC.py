...
#     - 0.4：Add DLC23 class; Add TRD class
#     - 0.5: Add DLC01 class
#     - 1.0: Generate script from a template DLC1.1_NTM_*mps.fst
#
# Description:
...
class DLC(object):
    ...
    def _fast(self, silence):
        ...
        if wrong is not False:
            ...
            filename = self.logPath.joinpath('{}.FAST.bug'.format(self.__id[-1]))
...
class DLC_turbulence(DLC):
    ...
    def _change_wind_profil(self):
        ...
        data = self._change_string(data, keyword='Filename')
        ...
        data = filename.read_text()
        data = self._change_string(data, keyword='InflowFile')
    ...
    def _change_string(self, text, keyword):
        ...
        if keyword == 'Filename':
            old="{}/{}_\*.bts".format(self.case, self.seed[0])
            new="{}/{}_{}mps_{}.bts".format(self.case,self.seed[0],self.seed[1],
                                            self.seed[2])
            text = re.sub(old, new ,text)

        if keyword == 'InflowFile':
            old="{}_{}_\*.IW.dat".format(self.case, self.seed[0])
...
class DLC13(DLC_turbulence):
    ...
    def __init__(self, seed, case='DLC1.3', outputFolder='', echo=False):
        super().__init__(seed, case, outputFolder, echo)
...
class DLC23(DLC):
    ...
    def __init__(self, gridloss, case='DLC2.3', outputFolder='', echo=False):
        ...
        if isinstance(self.gridloss[1], str):
            self.fastFile = '{}_{}_{}.fst'.format(self.case, self.gridloss[0],
                                                  self.gridloss[1])
        elif isinstance(self.gridloss[1], (float, int)):
            self.fastFile = '{}_{}_*.fst'.format(self.case, self.gridloss[0])
    ...
    def _change_gridloss(self):
        ...
        if isinstance(self.gridloss[1], str):
            filename = self.wtPath.joinpath('ServoDyn_{}_{}.dat'
                                           .format(self.case, self.gridloss[2]))
        elif isinstance(self.gridloss[1], (float, int)):
            filename = self.wtPath.joinpath('ServoDyn_{}_{}_{}.dat'.format(
                                 self.case, self.gridloss[1], self.gridloss[2]))
        # write to file
    ...
    def _change_string(self, text):
        if isinstance(self.gridloss[1], str):  
            ...
            old = "WT/ServoDyn_DLC2.3.dat"
            new = "WT/ServoDyn_{}_{}_{}.dat".format(self.case,
                                             self.gridloss[1], self.gridloss[2])
            temp = re.sub(old, new, text)
            
            old = "\?\?"
            new = str(self.gridloss[1])
            result = re.sub(old, new, temp)
        return result
...
def get_DLC13(seed, outputFolder='',  silence=False, echo=False):
    temp = DLC13(seed=seed, outputFolder=outputFolder, echo=echo)
    temp.run(silence=silence)
    return seed

def get_DLC13_multiprocess(list_seed, outputFolder='', silence=False,
                           echo=False):
    print('DLC v0.6 (December 31 2018)')
    print('========== Multiprocessing Mode ==========')
    # prepare a callback function
    length = len(list_seed)
    completed = []
    ...
    def printer(seed):
        ...
        rest = length - len(completed)
        hour, minute = time.strftime("%H,%M").split(',')
        print("|- [FAST][{}/{}] Simulation for {} wind at {} m/s with seed ID "
              "{} is terminated at {}:{}. {} tasks waiting to be completed ..."
              .format(pos, length, seed[0],seed[1],seed[2], hour, minute, rest))
    # begin multiprocessing
    pool = multiprocessing.Pool(CORES)
    [pool.apply_async(get_DLC13, args=(seed, outputFolder, silence, echo),
...
def main():
    ...
    # get_DLC11(seeds[0])
    get_DLC11(['NTM', '4.0', '1879136045'], silence=False)
    exit()
    ...
    get_DLC13(['ETM','3','-834366423'], silence=False)

    # ----- Running on multi processor
    # get_DLC23_multiprocess([['EOG', 'R', '77.6'], ['EOG', 'R', '77.8']], silence=1)
    get_DLC13_multiprocess([['ETM', '25', '2028409674'],
                           ['ETM', '25', '1960417032']], silence=True)
