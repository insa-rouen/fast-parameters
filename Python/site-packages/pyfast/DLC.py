...
class DLC(object):
    ...
    def __check_directory(self):
        ...
        self.outputPath.mkdir(exist_ok=True) # creat if non-existant
...
class TRD(DLC23):
    """ TRD
        ATTRIBUTES
            time: gridloss time [float]
    """
    def __init__(self, mode1, gridloss, case, outputFolder='', echo=False):
        ...
        # Change some fixed path
        self.__customize_path(outputFolder)
    ...
    def __customize_path(self, outputFolder):
        self.workPath = Path('~/Eolien/Parameters/Python/TRD').expanduser()
        self.outputPath=Path('~/Eolien/Parameters/Python/TRD/Output/{}_TRD'.format
                             (self.case), '{}'.format(outputFolder))
        self.outputPath = self.outputPath.expanduser()
        self.outputPath.mkdir(exist_ok=True)
        self.logPath=Path('~/Eolien/Parameters/Python/TRD/log'.format(self.case))
        self.logPath = self.logPath.expanduser()
    ...
    def _move(self):
        ...
        source = self.workPath.joinpath('{}_{}_{}_{}.out'.format(self.case,
                                 self.gridloss[0], self.gridloss[1], self.__id))
        destination = self.outputPath.joinpath('{}_{}_{}.out'.format(
                                 self.gridloss[0], self.gridloss[1], self.__id))
        source.replace(destination)
...
def get_DLC11_multiprocess(list_seed, outputFolder='', silence=False,
                           echo=False):
    print('DLC v0.4 (November 20 2018)')
...
def get_TRD(mode1, gridloss, case='DLC2.3', outputFolder='', silence=False, echo=False):
