...
class Analysis(object):
    ...
    def _load(self):
        ...
        self.dataLength = len(self.dataInput[s[1]]['MaxSpots'])
    ...
    def plotMaxDamageCountsByPie(self, toPDF=[]):
        ...
        for (ind,ax) in enumerate(axes):
            ...
            data = sorted(self.dataInput[v]['Occurence'], key=lambda x:x[1], reverse=True)
            labels = []
            fracs = []
            for x in data:
                fracs.append(x[1])
    ...
    def plotMaxDamageCountsByBar(self, gage=None, shareXaxis=True, shareYaxis=False, toPDF=[]):
        ...
        allNames = []
        for v in self.listOfSpeed:
            names = [x[0] for x in self.dataInput[v]['Occurence']]
        ...
        for (ind,ax) in enumerate(axes):
            v = self.listOfSpeed[ind]
            data = dict(self.dataInput[v]['Occurence'])
            names = allNames
            values = []
    ...
    def plotMaxDamageCountsByBar_gage1(self, shareXaxis=True, shareYaxis=False, toPDF=[]):
        figure = self.plotMaxDamageCountsByBar(gage='TwHt1', shareXaxis=shareXaxis, shareYaxis=shareYaxis)
        figure.suptitle('Counts of max damage at different wind speed ({} runs) : Tower Gage 1'.format(self.dataLength))
        chart.draw(figure, toPDF, left=0.03, bottom=0.1, right=0.97, top=0.94, wspace=0.12, hspace=0.12)
