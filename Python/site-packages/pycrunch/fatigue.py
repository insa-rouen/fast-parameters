...
class Analysis(object):
    ...
    def fitLognormOnSpot(self, speed, spot, bins=20):
        # origin data
        ...
        # N, nbins, patches = axis.hist(data, bins=bins,
        #                               weights=np.ones(len(data))/len(data))
        # # fitting distribution
        # param = scistats.lognorm.fit(N)
        # print(param)
        # x = scipy.linspace(min(data), max(data), num=bins)
        # pdf_fitted = scistats.lognorm.pdf(x, *param[:-2], loc=param[-2],
        #                                   scale=param[-1])
        # axis.plot(x, pdf_fitted, label="Log normal")
    ...
    def fitLognormOnGage(self, speed, gage):
        # origin data
        data = collections.deque()
        for theta in range(0, 360, 10):
            ...
            filename = Path("./tmp/{}.dat".format(theta))
