...
class Analysis(object):
    ...
    def fitOnGage(self, speed, gage, quantiles=(0.025, 0.25, 0.5, 0.75, 0.975),
                  echo=True):
        info_all = []
        data = collections.deque()
        if echo:
            print("\nSearching a best distribution for fatigue damages at wind "
                  "speed {} m/s on tower gage {}:".format(speed, gage))
        for theta in range(0, 360, 10):
            ...
            if echo:
                print("|- fitting {} observations on {} degree ...".format(
                      len(data), theta))
            dist_info = hbstats.bestfit(data, plotCDF=False, echo=False)
            # dist_all.append((speed, theta, dist_info))
            # find quantile values of distribution
            dist = dist_info[1]
            param = dist_info[2]
            res_dist = dist.ppf(quantiles, *param[:-2], loc=param[-2],
                                scale=param[-1])
            # find quantile values of observations
            res_obs = np.quantile(data, quantiles, interpolation="midpoint")
            # expectation_obs = np.mean(data)
            temp = [speed, theta, dist_info[0]]
            temp.extend(res_dist)
            temp.extend(res_obs)
            info_all.append(temp)
        
        title = ("Speed (m/s)", "Theta (°)", "Distribution",
                 "[Dist] Lower boundary {}%".format(quantiles[0]),
                 "[Dist] 1st quantile 25%", "[Dist] Median 50%",
                 "[Dist] 3rd quantile 75%",
                 "[Dist] Upper boundary {}%".format(quantiles[-1]),
                 "[Obser] Lower boundary {}%".format(quantiles[0]),
                 "[Obser] 1st quantile 25%", "[Obser] Median 50%",
                 "[Obser] 3rd quantile 75%",
                 "[Obser] Upper boundary {}%\n".format(quantiles[-1]))
    
        filename = "{}@{}mps.sts".format(len(data), speed)
        ...
        print("[OK] All data are saved to file {}".format(filename))
