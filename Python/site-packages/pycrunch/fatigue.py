...
class Analysis(object):
    ...
    def fitOnGage(self, speed, gage, distname=None, quantiles=(0.025, 0.25, 0.5,
                  0.75, 0.975), alert="error", saveDist=True, echo=True):
        info_all = []
        dist_all = []
        data = collections.deque()
        if echo:
        ...
        for theta in range(0, 360, 10):
            ...
            temp = [speed, theta, dist_info[0][0]]
        ...
        title = ("Speed (m/s)", "Theta (°)", "Distribution",
                 ...
                 "[Obser] Upper boundary {}%\n".format(quantiles[-1]*100))
...
class Lifetime(object):
    ...
    """
    def __init__(self, measure_file, measure_duration, wind_speed_range,
        distribution_directory, years, simulated_time):
        rawdata = pd.read_csv(measure_file, delimiter=";", header=None)
        self.measure_data = rawdata.iloc[:, [3, 2]].copy()
    ...
    def select_speed_range(self, speed, show=False):
        ...
        if not df.any(axis=None).speed: # if wind doesn't occur in this range
            ...
            # cumulative appearance in each
            wd_count = np.sum(ax._info['table'], axis=0)
            # direction
            wd_time = wd_count * self.measure_duration
    ...
    def calculate_damage_speed(self, speed, damages_direction, show=False):
        ...
        if show:
            ...
            axis.set_theta_direction(-1)  # set angle increasing clockwise
            axis.set_thetagrids(range(0, 360, 30))  # change theta gridlines
            # theta = pd.DataFrame(list(range(0, 360, 10)))
            X = np.radians(self.theta)
            X = np.append(X, X[0])
    ...
    def evaluate_fatige_life(self, show=False):
        ...
        if show:
            ...
            axis.legend()
            axis.set_title("Cumulative fatigue for {} years".format(years))
            xlabels = ('N', '', 'N-E', '', 'E', '', 'S-E', '', 'S', '', 'S-W',
                       '', 'W', '', 'N-W', '',)
            axis.set_xticklabels(xlabels)
            plt.show()
...
def main():
    ...
    if essai == 3:
        ...
        measure_duration=30, wind_speed_range=range(3,27,2),
        distribution_directory="~/PhD/Memo/DLC1.1b/Figures/10000seeds",
        years=20, simulated_time=10)
        
        wd_time = test.select_speed_range(speed=17, show=0)
        print(wd_time)
        unit_dam = test.load_sts(speed=17)
        print(unit_dam)
        dam_dir = test.calculate_damage_direction(wind_direction_time=wd_time,
            unit_damage=unit_dam, show=0)
        print(dam_dir)
        test.calculate_damage_speed(speed=17, damages_direction=dam_dir, show=1)
    ...
    if essai == 33:
        ...
        for year in X:
            ...
            test = Lifetime(
                ...
                measure_duration=30, wind_speed_range=range(3, 27, 2),
                distribution_directory="~/PhD/Memo/DLC1.1b/Figures/10000seeds",
                years=year, simulated_time=10)

            fatigue = test.evaluate_fatige_life(show=0)
            print("Fatigue =", fatigue.values)
            Y.extend(fatigue)
        plt.plot(X,Y, 'o-')
        plt.show()
