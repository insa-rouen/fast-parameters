...
class Aster1(object):
    ...
    def resume(self, programName, inputFileSize=None, outputFileSize=None):
        ...
        if programName.lower() == 'FAST'.lower():
            ...
            inputList = self._find(self.windPath, '*.out', inputFileSize)
            recompute = list(set(inputList).difference(outputList))
            self._convertToSeed(recompute)
            print('|- {} tasks will be recomputed by Stree'.format(len(recompute)))
        elif programName.lower() == 'Fatigue'.lower():
            outputList = self._find(self.outputPath, '*.dam', outputFileSize)
            inputList = self._find(self.windPath, '*.str', inputFileSize)
            recompute = list(set(inputList).difference(outputList))
            self._convertToSeed(recompute)
            print('|- {} tasks will be recomputed by Fatigue'.format(len(recompute)))
            ...
            with open('recomputeSeeds.json','w') as f:
                f.write(encode)
