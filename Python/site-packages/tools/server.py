...
class Aster1(object):
    ''' aster1 server
        ...
        *METHODS*
            get():
    '''
    def __init__(self, inputSeeds, windPath, outputPath, echo):
        self.inputSeeds = inputSeeds
    ...
    def resume(self, programName, inputFileSize=None, outputFileSize=None, compress=False):
        ...
        if programName.lower() == 'FAST'.lower():
            ...
            inputList = ['{}_{}mps_{}'.format(s[0], s[1], s[2]) for s in self.inputSeeds]
            recompute = list(set(inputList).difference(set(outputList)))
            self._convertToSeed(recompute)
            # write to file
            encode = json.dumps(self.seeds, indent=4)
            p = Path(self.windPath).parent.expanduser().joinpath('recomputeTurbSim.json')
            p.open('w').write(encode)
            print('|- {} tasks need to be recomputed by TurbSim'.format(len(recompute)))
        elif programName.lower() == 'ALL'.lower():
            outputList = self._find(self.outputPath, '*.dam', outputFileSize)
            inputList = ['{}_{}mps_{}'.format(s[0], s[1], s[2]) for s in self.inputSeeds]
            recompute = list(set(inputList).difference(set(outputList)))
            self._convertToSeed(recompute)
             # write to file
            encode = json.dumps(self.seeds, indent=4)
            p = Path(self.windPath).parent.expanduser().joinpath('recomputeALL.json')
            p.open('w').write(encode)
            print('|- {} tasks need to be totally recomputed'.format(len(recompute)))
        else:
        ...
        if compress:
            ...
            print('|- {} {} files will be compressed'.format(len(finished), extension))
    ...
    def finalcheck(self, btsFileSize=None, outFileSize=None, tgzFileSize=None, strFileSize=None, damFileSize=None):
        inputList = ['{}_{}mps_{}'.format(s[0], s[1], s[2]) for s in self.inputSeeds]
        btsList = self._find(self.windPath, '*.bts', btsFileSize)
        outList = self._find(self.outputPath, '*.out', outFileSize)
        tgzList = self._find(self.outputPath, '*.out.tgz', tgzFileSize)
        # strList = self._find(self.outputPath, '*.str', strFileSize)
        damList = self._find(self.outputPath, '*.dam', damFileSize)
        
        # Total number of tasks
        print('Total submitted tasks             : {}'.format(len(inputList)))
        print('|- TurbSim completed tasks (.bts) : {}'.format(len(btsList)))
        print('|- FAST completed tasks (.out)    : {}'.format(len(outList)))
        if tgzFileSize is not None:
            print(' |- compressed (.out.tgz)         : {}'.format(len(tgzList)))
        # print('|- Stress completed tasks (.str)  : {}'.format(len(strList)))
        print('|- Fatigue completed tasks (.dam) : {}'.format(len(tgzList)))

        # Check difference
        btsDiff = set(inputList).difference(set(btsList))
        outDiff = set(inputList).difference(set(outList))
        tgzDiff = set(inputList).difference(set(tgzList))
        # strDiff = set(inputList).difference(set(strList))
        damDiff = set(inputList).difference(set(damList))
        print('-----------------------------------------------')
        print('|- {} tasks of TurbSim (.bts) is not found       : {}'.format(len(btsDiff), btsDiff))
        print('|- {} tasks of FAST (.out) is not found'.format(len(outDiff)))
        if tgzFileSize is not None:
            print(' |- {} compressed result (.out.tgz) is not found : {}'.format(len(tgzDiff), tgzDiff))
        # print('|- {} tasks of Stress (.str) is not found'.format(len(strDiff)))
        print('|- {} tasks of Fatigue (.dam) is not found       : {}'.format(len(damDiff), damDiff))

    def sendmail(self, receiver):
        with utils.cd('~/Eolien/Parameters/Python/site-packages/tools'):
            gmail.send(receiver=receiver, servername=self.__nodename) 
