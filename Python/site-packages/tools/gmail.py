...
def send(data, by='gmail', language='zh-cn', debug=False):
    ...
    #*============================== Sending mail ==============================
    # Method 1: via Hotmail
    if by.lower() == 'hotmail':
        import smtplib
        username = 'cwuhuang@hotmail.com'
        password = 'changwu&hao_2020'
        sender = 'cwuhuang@hotmail.com'
        ...
        try:
            server.sendmail(sender, receiver, messageRoot.as_string())
        except:
            ts = time.strftime("(%Z%z) %Y-%m-%d %H:%M:%S", time.localtime())
            print("|- [ERROR] Sending email failed at {} !".format(ts))
            raise
        else:
            ts = time.strftime("(%Z%z) %Y-%m-%d %H:%M:%S", time.localtime())
            print("|- [OK] Mail sent at {} !".format(ts))
        finally:
            server.quit()
