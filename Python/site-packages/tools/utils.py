...
def timer(function):
    ...
    def wrapper(*args, **kwargs):
        ...
        tok = time.time()
        print("[INFO] {}() finished in {:.3f}".format(function.__name__, tok-tik))
        return result
    return wrapper
...
def compress(filename, removeSource):
    if removeSource:
        command = 'tar -cvzf {0}.tgz {0} && rm {0}'.format(filename)
    else:
        command = 'tar -cvzf {0}.tgz {0}'.format(filename)
    try:
