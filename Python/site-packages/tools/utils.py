...
def find(keyword, path, N=None, size=None):
    ''' Search files by a given keyword in a directory
        N: int
            number of files expected to return
        size: float
            minimum size in bytes (1 GB = 1024 MB = 1024^2 KB = 1024^3 Bytes)
    '''
    p = Path(path)
    if not p.exists():
        raise FileNotFoundError("No such directory: '{}'".format(path))
    list_file = sorted(p.glob(keyword))
    # filter: numbers of file
    if len(list_file) == 0:
        raise FileNotFoundError("Nothing has been found for the keyword '{}' in"
                                " the directory '{}'".format(keyword, path))
    elif (N is not None) and (len(list_file) != N):
        raise EnvironmentError("{} files expected, but only {} files are found"
                               " for the keyword '{}' in the directory {}"
                               .format(N, len(list_file), keyword, path))
    # filter: file minimum size
    if size is not None:
        list_file = [x for x in list_file if x.stat().st_size >= size]
    return list_file
