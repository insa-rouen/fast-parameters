...
def readcsv(filename, delimiter='\t', header=7, datarow=9, unitrow=None,
            endrow=None, colnum=None, echo=True):
    ...
    if datarow-header >= 2:
        ...
        if unitrow is not None:
            ...
            if endrow is not None: # ignore data after endrow (not included)
                length = endrow - datarow + 2
                dataframe = dataframe[1:length]
        else:
            dataframe.drop(list(range(endIndex)), inplace=True)
            if endrow is not None: # ignore data after endrow (not included)
                length = endrow - datarow
    ...
    for i in range(size):
        ...
        if math.isnan(sum(records)):
            raise Warning("[ALERT] NaN value is found in {}".format(filename))
