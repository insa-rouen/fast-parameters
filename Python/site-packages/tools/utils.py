...
def cp(source, destination):
    """ Copy a file
        INPUT
            source: origin file [str/Path]
            source: target file [str/Path]
    """
    ...
    if source.is_file():
        ...
        raise ValueError("[ERROR] Fail in copy {} to {}. The source must be a "
                         "file or a directory".format(source, destination))
        ...
        if checkNaN:
            if math.isnan(sum(records)):
                ...
                for (key,value) in enumerate(records):
                    if math.isnan(value):
                        if j <= 5:
                            print("[ALERT] The value of {} on position #{}, " 
                                "column #{} is NaN or empty !".format(value, 
                                key, i))
                            j = j + 1
                        else:
                            break
                message = "[ALERT] NaN value is found in file {}".format(
                    filename, i, name)
                if checkNaN == "warning":
                    print(message)
                    return None
