...
class Count(object):
    ...
    '''
    def __init__(self, filename, gages=None, echo=False):
    ...
    def run(self):
        ...
        if self.echo: print("|- Importing {} ...".format(self.filename))
        self.open()
    ...
    def open(self):
        self.dataInput = utils.readcsv(self.filename, unitrow=8, echo=self.echo)
        ...
        if self.gages is None: # count cycles on every spot over each tower gage
            ...
            gages = self.dataInput.getkeys()
            gages.pop(0) # remove "Time" column
            self.gages = gages
        elif isinstance(self.gages[0], int):
            gages = []
            for spot in self.gages:
...
class Fatigue(object):
    ''' Calculate cumulative damage
        *ATTRIBUTES*
    '''
    def __init__(self, filename, gages=None):
        if gages is None: #[Default] run rainflow counting on every local spots
            self.RFdata = Count(self.fatigueStress)
        elif isinstance(gages[0], int): # read a serie of gage nodes 
            self.RFdata = Count(filename, gages)
        elif isinstance(gages[0], str): # read a list of local spots
            self.RFdata = Count(filename, gages)
        else:
        ...
        self.filename = filename
        self.gages = self.RFdata.rainflowData.keys()
        # self.gages.sort()
        self.damage = dict.fromkeys(self.gages)
    ...
    def assessAllStressSpot(self):
        ...
        list(map(self.assessStressSpot, self.gages))
    ...
    #TODO
    def assess_multiprocess(self,):
        # pool = multiprocessing.Pool()
        pool = multiprocessing.pool.ThreadPool()
        # [pool.apply_async(self.assessStressSpot, args=(spot,)) for spot in self.gages]
        pool.map_async(self.assessStressSpot, self.gages)
        pool.close()
        pool.join()

    def assessStressSpot(self, spot):
    ...
    def save(self, filename=None):
        if filename is None: filename = self.filename + ".dmg"
...
def main():
    ...
    # Testing Count class
    if test == 2:
        myCount3 = Count('DLC1.2_NTM_25mps.str', gages=[1,5,9])

    # Tesint Fatigue class
    if test == 3:
        analysis = Fatigue('DLC1.2_NTM_25mps.str', [1,9])
        analysis.assessAllStressSpot()
        # analysis.assess_multiprocess()
        # analysis.assessStressSpot('TwHt1@0')
        # analysis.save()
