...
class Count(object):
    ...
    def open(self):
        ...
        if isinstance(self.filebase, str):
            self.dataInput = utils.readcsv(self.filebase+'.str', unitrow=8, echo=self.echo)
...
class Fatigue(object):
    ...
    def assess_multithread(self):
        ''' Try to make calculation in parallel
            Note: proved to be USELESS
        '''
...
def main():
    ...
    if test == 2:
        ...
        # myCount2 = Count('DLC1.2_NTM_25mps', gages=[1,5,9])
        # myCount3 = Count('DLC1.2_NTM_25mps')
        test = meca.get_stress('DLC1.2_NTM_25mps', 6009, [1,9], thetaStep=30)
        myCount4 = Count(test)
    ...
    if test == 3:
        ...
        # analysis.save()
        # get_fatigue('DLC1.2_NTM_25mps', [1,2,3,4,5,6,7,8,9])
