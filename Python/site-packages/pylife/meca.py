...
class Stress(object):
    ...
    '''
    def __init__(self, filename, datarow, gage, thetaStep, startline, echo):
    ...
    def run(self):
        if self.echo: print("Nominal stress v0.3 (October 19 2018)")
        self.__geometry()
        if self.echo: print("|- Importing "+self.filenameInput+" ...")
        self.open()
...
#!                                      FUNCTIONS
#!----------------------------------------------------------------------------------------
def get_stress(filename, datarow, gage, thetaStep=30, startline=7, echo=False):
    temp = Stress(filename=filename, datarow=datarow, gage=gage, thetaStep=thetaStep, startline=startline,
                  echo=echo)

def get_stress_multiprocess(list_filename, datarow, gage, thetaStep=30, startline=7, echo=False):
    pool = multiprocessing.Pool()
    [pool.apply_async(get_stress, args=(filename, datarow, gage, thetaStep, startline, echo)) for filename in list_filename]
    pool.close()
    pool.join()
...
def main():
    tik=time.time()
    mydata=Stress(filename='DLC1.2_NTM_25mps.out', datarow=6009, gage=[1,9], thetaStep=30, startline=7, echo=False)
    tok=time.time()
    print(tok-tik)
