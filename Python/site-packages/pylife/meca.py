...
class Stress(object):
    ...
    '''
    def __init__(self, filename, gage, thetaStep, startline, echo):
...
#!                                      FUNCTIONS
#!----------------------------------------------------------------------------------------
def get_stress(filename, gage, thetaStep=30, startline=7, echo=False):
    temp = Stress(filename=filename, gage=gage, thetaStep=thetaStep, startline=startline,
                  echo=echo)

def get_stress_multiprocess(list_filename, gage, thetaStep=30, startline=7, echo=False):
    pool = multiprocessing.Pool()
    [pool.apply_async(get_stress, args=(filename, gage, thetaStep, startline, echo)) for filename in list_filename]
    pool.close()
    pool.join()
