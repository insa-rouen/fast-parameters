...
function upper_mean(file)
    # read data from file
    data = @time CSV.read(file, header=7, datarow=6009, delim="\t")
    ...
    return means, header, colnumbers
end

function main(files)
    # initiate DataFrame
    mean, header, colnumbers = upper_mean(files[1])
    df = CSV.DataFrame(["string";(1.0:colnumbers)], [:File;header])
    # apply to all files
    for file in files
        mean, header, colnumbers = upper_mean(file)
        push!(df, [file;mean])
    end
    # remove the 1st line
    CSV.DataFrames.deleterows!(df,1)
    # write to file
    CSV.write("CharacteristicLoads.mean", df; delim='\t')    
end
